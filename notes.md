...
# Issue #1 Summary: Main Points & Requested Changes

## Layout Problems
- Images are displayed in a single column, not a grid.
- UI is cluttered, requiring excessive scrolling.
- Suggestion: Add a tab system or better layout for easier editing and overview.
- The "photoedit fusion" tab can be removed.

## Performance
- App is slow, laggy, and prone to freezing (e.g., clipping hangs at 12%).

## Background Remover
- Results are unpredictable: sometimes removes too much, too little, or creates malformed outputs.

## Clipboard/Restore
- Suggestion: Add a clipboard feature or a way to restore the last project.

## Export Issues
- Export is unreliable—either not working or saving silently.
- Suggestion: Show a visible success/failure notification after export.

## Reference Tool
- Not working at all.

## Resize Frame
- The orange resize frame should be transformable.

## Split Tool
- Remove the "split in 4" function from the batch editor tool, but keep the standalone split tool.

## Showcase Preview
- Preview is too large, even with small tiles and little spacing (screenshot provided).

## Heaviest Issues
- Layout and performance are the most critical problems.

---

## Fusion

alle standalone tools zusammen in die photoedit.html im root integrieren

## Zwischenspeicher 

ich brauche einen Zwischenspeicher. wenn ich leere Kacheln einfüge resettet sich die Reihenfolge der Buchstaben wenn ich sie geändert hab. 

## croppen/Resize/relation

Funktion um mehrere oder einzelne Buchstaben zuzuschneiden 
Eine Funktion um alle Buchstaben in ein transparentes quadrat zu packen
